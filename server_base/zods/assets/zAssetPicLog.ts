...
export const zAssetPicLogOutput = z.object({_id:zObjectId(),document:zAssetPicOutput.or(zObjectId()).optional(),triggerBy:zUserOutput.or(zObjectId()).optional().nullable(),operation:z.string().optional(),changeData:z.any().optional()});
export const zAssetPicLogQuery = z.object({document:ZodMongoQuery.z$query(zObjectId()),triggerBy:ZodMongoQuery.z$query(zObjectId()),operation:ZodMongoQuery.z$query(z.string()),changeData:ZodMongoQuery.z$query(z.any())}).passthrough().partial();
export const zAssetPicLogInput = z.object({document:zObjectId().optional(),triggerBy:zObjectId().optional().nullable(),operation:z.string().optional(),changeData:z.any().optional()});
export const ZodAssetPicLogAPIs = getBasicRouteZodIO({
  input: zAssetPicLogInput,
  output: zAssetPicLogOutput,
  query: zAssetPicLogQuery,
},zAssetPicLogOutput.keyof());