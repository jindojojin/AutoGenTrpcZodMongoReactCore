...
export const zAssetInvoiceLogOutput = z.object({_id:zObjectId(),document:zAssetInvoiceOutput.or(zObjectId()).optional(),triggerBy:zUserOutput.or(zObjectId()).optional().nullable(),operation:z.string().optional(),changeData:z.any().optional()});
export const zAssetInvoiceLogQuery = z.object({document:ZodMongoQuery.z$query(zObjectId()),triggerBy:ZodMongoQuery.z$query(zObjectId()),operation:ZodMongoQuery.z$query(z.string()),changeData:ZodMongoQuery.z$query(z.any())}).passthrough().partial();
export const zAssetInvoiceLogInput = z.object({document:zObjectId().optional(),triggerBy:zObjectId().optional().nullable(),operation:z.string().optional(),changeData:z.any().optional()});
export const ZodAssetInvoiceLogAPIs = getBasicRouteZodIO({
  input: zAssetInvoiceLogInput,
  output: zAssetInvoiceLogOutput,
  query: zAssetInvoiceLogQuery,
},zAssetInvoiceLogOutput.keyof());