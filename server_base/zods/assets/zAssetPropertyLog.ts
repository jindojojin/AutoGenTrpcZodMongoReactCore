...
export const zAssetPropertyLogOutput = z.object({_id:zObjectId(),document:zAssetPropertyOutput.or(zObjectId()).optional(),triggerBy:zUserOutput.or(zObjectId()).optional().nullable(),operation:z.string().optional(),changeData:z.any().optional()});
export const zAssetPropertyLogQuery = z.object({document:ZodMongoQuery.z$query(zObjectId()),triggerBy:ZodMongoQuery.z$query(zObjectId()),operation:ZodMongoQuery.z$query(z.string()),changeData:ZodMongoQuery.z$query(z.any())}).passthrough().partial();
export const zAssetPropertyLogInput = z.object({document:zObjectId().optional(),triggerBy:zObjectId().optional().nullable(),operation:z.string().optional(),changeData:z.any().optional()});
export const ZodAssetPropertyLogAPIs = getBasicRouteZodIO({
  input: zAssetPropertyLogInput,
  output: zAssetPropertyLogOutput,
  query: zAssetPropertyLogQuery,
},zAssetPropertyLogOutput.keyof());